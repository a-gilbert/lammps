...
PairSPHEGas::PairSPHEGas(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
}
...
void PairSPHEGas::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        // artificial viscosity (Monaghan 1992) TURNED OFF D:
        // if (delVdotDelR < 0.) {
...
void PairSPHEGas::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHEGas::coeff(int narg, char **arg) {
  if (narg != 4)
...
double PairSPHEGas::init_one(int i, int j) {
...
double PairSPHEGas::single(int i, int j, int itype, int jtype,
